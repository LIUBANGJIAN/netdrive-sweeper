...
> 当前版本：**1.1.0**（页面左上角标题旁会显示版本角标；版本号单一来源为仓库根 `VERSION` 文件，
> Docker/CI 构建时经 `-ldflags -X main.appVersion=...` 注入，从 git 检出直接 `go build` 时还会附上提交短 SHA）
...
3. **事件驱动而非轮询**：常驻订阅 `PushMessage` 流，收到 `FILE_SYSTEM_CHANGE` 事件后**防抖合并**再触发扫描。**禁止**任何「每 N 秒扫全树」的循环。
   ...
   - 订阅的真实状态（`已连接` / `已连接（无文件事件）` / `权限不足` / `未启用` / `连接失败`）由 `/api/state` 与 `/api/push`
     暴露给页面，因此「事件驱动到底有没有在跑」不再需要靠猜或翻日志。
   - **两级兜底，保证文件事件断流时依然会清理**（都不遍历全树，只做有界检查）：
     - **离线任务监控**（`offline_monitor_minutes`，默认 **1** 分钟）：每轮只对每个清理目录做 1 次
       轻量离线状态查询（`ListOfflineFilesByPath`），检测到「下载中 → 完成」翻转即触发一次扫描。
       目标是把响应从 15 分钟缩短到约 1 分钟。运行态（是否启用、间隔、最近检查时间、累计触发次数、
       当前下载中的目录）通过 `/api/state`、`/api/push` 的 `offlineMonitor` 字段暴露到页面。
     - **事件静默兜底扫描**（`event_fallback_scan_minutes`，默认 **15** 分钟）：事件驱动运行时，
       若连续 N 分钟未收到任何文件变更事件，自动执行一次兜底扫描。文件事件长期断流时，
       它**等价于「每 N 分钟扫描一次」的有界轮询**，而非无界扫全树。
   - **关于 `isCloudEventListenerRunning=false`**：该字段为 CD2 服务端只读状态，**在部分 CD2 版本上对所有云盘
     恒为 false，属常态、并非故障**，外部无法通过重启或改配置「修复」。因此本工具不再给出
     「检查云盘连接 / 重启 CD2」这类无效排查动作，而是如实提示「未上报云端事件通道」，
     并说明实时清理已由上述两级兜底自动接替。
...
事件断流时的两级兜底（有界检查，不遍历全树）:
  ① 离线任务监控（默认每 1 分钟）:
       对每个清理目录查询离线状态 ──► 检测到「下载中 → 完成」翻转 ──► 触发一次扫描
  ② 事件静默兜底扫描（默认每 15 分钟）:
       连续 N 分钟未收到任何文件变更事件 ──► 触发一次扫描

手动触发:
  Web 页面「手动清理」──► 走同一套扫描与校验逻辑
```
...
```bash
# 从 git 检出构建：页面版本角标为 v<VERSION> (<提交短SHA>)
go build -o netdrive-sweeper .

# 或显式注入版本（与仓库根 VERSION 保持一致）
go build -ldflags "-X main.appVersion=$(cat VERSION)" -o netdrive-sweeper .
./netdrive-sweeper
```
...
页面只有两个页签：**① 连接 · 目录 · 规则**（配置）与 **② 运行日志**（执行与查看）。
**所有配置项保存后立即生效——包括事件驱动订阅与两级兜底，无需重启容器。**
...
6. **打开删除总开关**「允许自动清理」——这是所有删除动作的前提。
   未开启时「手动清理」只做扫描预览，不会删除任何文件。
7. **点「保存配置」**：配置落盘，事件驱动订阅与两级兜底按新配置自动重启/热生效（页顶「事件驱动」状态会变为`已连接`）。
8. 切到 **② 运行日志**，点「**手动清理**」：开启删除总开关时会先弹确认框，
   确认后**同一轮内完成扫描与删除**；结果、清理记录与运行日志都在本页，并带执行时间。

> ⚠️ 建议先在**少量测试目录**上验证规则，确认无误后再放开到全盘。
> 💡 页首「事件驱动」状态是权威状态源：`已连接`=订阅正常且在收文件事件；`已连接（无文件事件）`=订阅正常
> 但近期没收到文件事件（由两级兜底接替，可正常使用）；`权限不足`=Token 缺
> `allow_push_message`（去 CD2 补上后回页面保存即可，无需重启）；`连接失败`=CD2 不可达。
> 💡 「运行日志」页还会显示**离线任务监控**状态（是否开启、间隔、最近检查时间、累计触发次数、
> 当前正在下载的目录），可直接判断它「有没有在跑」，而不必只看有没有日志。
...
| `event_scan_min_interval_minutes` | `5` | 两次事件驱动扫描之间的**最小间隔**（分钟），防止无关变更把网盘 API 刷爆；`0` = 关闭冷却 |
| `event_fallback_scan_minutes` | `15` | **事件静默兜底扫描**间隔（分钟）：连续 N 分钟未收到任何文件变更事件即扫描一次；`0` = 关闭兜底 |
| `offline_monitor_minutes` | `1` | **离线任务监控**间隔（分钟）：检测清理目录的「下载中 → 完成」翻转即触发扫描；`0` = 关闭监控。上限自动收敛为 60 |
| `config_version` | `4` | 内部字段：配置迁移版本。旧配置（无此字段或版本落后）启动时执行一次性迁移，并逐级补齐新字段（如把旧默认 `file_cooldown_hours=6` 改为 `0`、补齐上述三项间隔的默认值） |

环境变量：`LISTEN`、`CONFIG_PATH`、`RECORDS_PATH`、`LOG_PATH`。

> ⚠️ **升级提醒（存量实例）**：从旧版本升级时，必须**重新构建/拉取新镜像并重启容器**，
> 启动日志会打印「配置迁移 vN→vM」与「兜底策略 | 离线任务监控=… 事件静默兜底=…」两行；
> 若日志中看不到这两行，说明运行的仍是旧二进制。此外，若 `tasks` 为空或 `token` 为空，
> 任何自动清理都不会发生（空目录 = 不扫描，这是刻意设计，不会隐式扫全盘）。
...
| 勾选了清理但没删掉 | 删除总开关 `allow_delete` 未开（此时「手动清理」只预览不删）；或 `file_cooldown_hours` 大于 0 且文件仍在冷却期（含刚完成的离线下载）；或目录含未完成后缀/离线任务未完成 |
| 事件驱动不生效 | 看页首「事件驱动」状态：`权限不足`=Token 缺 `allow_push_message`（补上后保存配置即生效，**无需重启容器**）；`未启用`=`enable_push` 关闭或地址/Token 为空；`连接失败`=CD2 不可达 |
| 提示「未上报云端事件通道」 | **这是部分 CD2 版本的常态、并非故障**（`isCloudEventListenerRunning` 为服务端只读字段，可能对所有云盘恒为 false）。实时清理已由「离线任务监控」+「事件静默兜底扫描」自动接替，也可随时「手动清理」。**不需要检查云盘连接，也不需要重启 CD2** |
| 页首显示「已连接（无文件事件）」 | 订阅正常，但近期未收到 `FILE_SYSTEM_CHANGE`。清理由两级兜底接替，属可正常使用的状态；若长期如此，可把「离线任务监控」间隔调小以加快响应 |
| 离线任务监控「没有日志 / 没生效」 | ① 先看「运行日志」页的**离线任务监控状态条**：若显示「已关闭（间隔设为 0）」→ 把「离线任务监控（分钟）」填 `1` 并保存；若显示「无清理目录」→ 先添加清理目录；② 若状态条显示「已开启 · 每 N 分钟检查一次」而只是没有触发日志，说明确实在跑、只是恰好没有目录发生「下载中 → 完成」翻转（触发时才写「触发扫描」日志）；③ 若整页**完全没有**「配置迁移」「兜底策略」等启动日志，则是**部署陈旧**，需重新构建/拉取镜像并重启容器 |
| 加了离线任务却没有任何动作 | ① 页首「事件驱动」是否为`已连接`（或`已连接（无文件事件）`）；② 「允许自动清理」是否已勾选；③ `file_cooldown_hours` 是否为 `0`；④ 「离线任务监控」是否已开启。日志中会打印收到的 `messageType`，可据此确认 CD2 实际发出的推送类型 |
| 日志比之前少 | 已不再对「地址/Token 未配置」每 10s 刷一条重连失败日志（改为单条状态提示）；后台动作会打「事件驱动实时清理已启动」「收到文件系统变更事件」「离线任务监控已启动」等日志 |
...
```
.
├── main.go          # HTTP 服务、配置读写、扫描入口、PushMessage 消费装配、离线任务监控循环
├── cd2client.go     # CD2 gRPC 客户端（dynamicpb + protocompile 动态解析 cd2.proto）
├── config.go        # 配置结构、默认值、归一化、持久化、版本迁移
├── limiter.go       # 令牌桶限速
...
├── VERSION          # 版本号单一来源（页面角标 / 镜像标签 / 构建注入均取此值）
├── main_test.go     # 单元测试
├── offline_monitor_test.go          # 离线任务监控（offlineCompleted / 配置迁移 / 默认值）
├── observability_version_test.go    # 版本角标 + 离线监控可观测性（API 字段 / 状态机 / 日志）
├── Dockerfile
├── docker-compose.yml
└── .github/workflows/docker-build.yml   # CI：从 VERSION 注入版本，构建并推送镜像到 Docker Hub
```
