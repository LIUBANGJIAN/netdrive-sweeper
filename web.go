...
async function gatherCfg(){return {address:val('address'),token:val('token'),ad_exts:val('adExts'),video_exts:val('videoExts'),size_limit_mb:parseFloat(val('sizeLimit')||0),exclude_dirs:val('excludeDirs'),ops_per_sec:parseFloat(val('opsPerSec')||5),file_cooldown_hours:parseInt(val('cooldown')||0),enable_push:checked('enablePush'),push_debounce_seconds:parseInt(val('pushDebounce')||5),force_refresh:checked('forceRefresh'),offline_only:checked('offlineOnly'),delete_permanently:checked('deletePermanently'),allow_delete:checked('allowDelete'),tasks:taskList()}}
