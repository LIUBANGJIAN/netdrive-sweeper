...
<label>清理目录，API 路径，一行一个；/ 表示 Token 授权根</label><textarea id="tasks"></textarea>
<div class="row"><label><input id="forceRefresh" type="checkbox" style="width:auto"> 强制刷新 CD2 缓存</label><label><input id="deletePermanently" type="checkbox" style="width:auto"> 永久删除</label></div>
...
document.getElementById('saveBtn').onclick=saveCfg;document.getElementById('testBtn').onclick=testConn;document.getElementById('scanBtn').onclick=function(){scan(false)};document.getElementById('cleanBtn').onclick=function(){scan(true)};document.getElementById('listBtn').onclick=listDir;document.getElementById('logsBtn').onclick=loadLogs;document.getElementById('clearLogsBtn').onclick=clearLogs;
